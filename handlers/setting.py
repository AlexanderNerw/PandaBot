...
async def inline_menu_setting(c: CallbackQuery) -> None:
    ...
    except Exception as ex: await exceptions("setting.py", 'inline_menu_setting', ex)
...
async def inline_menu_setting_gender(c: CallbackQuery) -> None:
    ...
    except Exception as ex: await exceptions("setting.py", 'inline_menu_setting_language', ex)
...
async def inline_menu_setting_language(c: CallbackQuery) -> None:
    try:
        ...
        await c.answer(reverse_info[f"{db.getting(c.message.chat.id, 'language')}_lang"]), await inline_menu_setting(c)
...
async def inline_menu_setting_name(c: CallbackQuery) -> None:
    ...
    except Exception as ex: await exceptions("setting.py", 'inline_menu_setting_name', ex)

@dp.message_handler(content_types='text', state=SetName)
async def set_name(message: Message, state: FSMContext) -> None: 
    try:
        if message.text == '/menu': await bot.send_message(f"{db.getting(message.chat.id, 'language')}_ask_admin_no", reply_markup=ReplyKeyboardRemove())
