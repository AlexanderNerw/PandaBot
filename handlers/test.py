from support.config import dp, bot, CHAT_PRIVATE, ADMIN, exceptions
...
async def question_back(call: CallbackQuery, state: FSMContext):
    try:
        ...
        try:
            ...
            await test_menu(call.message.chat.id, call.message.message_id+1)

    except Exception as ex: exceptions('test.py', 'question_back', ex)
...
try: # TEST DEPRESSION BEKA 
    ...
    async def menu_test_depression_beka(call: CallbackQuery) -> None:
        ...
        except Exception as ex: await exceptions("test.py", 'menu_test_depression_beka', ex)
    ...
    async def questions_TDB(call: CallbackQuery, state: FSMContext): 
        try:
            ...
            if str(ex) != 'Message to delete not found':
                await exceptions("test.py", 'questions_TDB', ex)
#=============================================== 
except Exception as ex: # TEST DEPRESSION BEKA 
    exceptions('test.py', 'TDBeka', ex)
...
try:  # ТЕСТ ТРЕВОЖНОСТИ БЕКА
    ...
    async def menu_test_worry_beka(call: CallbackQuery) -> None:
        ...
        except Exception as ex: await exceptions("test.py", 'menu_test_worry_beka', ex)        
    ...
    async def question1_TTB(call: CallbackQuery, state: FSMContext):
        ...
        except Exception as ex: await exceptions("test.py", 'question1_TTB', ex)
    ...
    async def questions_TTB(call: CallbackQuery, state: FSMContext): 
        try:
            ...
            if str(ex) != 'Message to delete not found':
                await exceptions("test.py", 'questions_TTB', ex)

        finally: await call.answer()
#===============================================        
except Exception as ex: # TEST DEPRESSION BEKA 
    exceptions('test.py', 'TTBeka', ex)
...
try:  # ТЕСТ БЕЗНАДЁЖНОСТИ БЕКА
    ...
    async def menu_test_hopeless_beka(call: CallbackQuery) -> None:
        ...
        except Exception as ex: await exceptions("test.py", 'menu_test_hopeless_beka', ex)
    ...
    async def question1_TBB(call: CallbackQuery, state: FSMContext):
        ...
        except Exception as ex: await exceptions("test.py", 'question1_TBB', ex)
    ...
    async def questions_TBB(call: CallbackQuery, state: FSMContext): 
        try:
            ...
            if str(ex) != 'Message to delete not found':
                await exceptions("test.py", 'questions_TBB', ex)

        finally: await call.answer()
#=============================================== 
except Exception as ex: # TEST DEPRESSION BEKA
    exceptions('test.py', 'TBBeka', ex)


############################################################################################### - МЕНЮ ТЕСТОВ
async def test_menu(chat_id, message_id):
    try:
        await bot.delete_message(chat_id, message_id)
        await bot.send_message(chat_id, f"🧾 " + general_text[f"{db.getting(chat_id, 'language')}_list_test"],
        parse_mode='html', reply_markup=menu_all_test[db.getting(chat_id, 'language')])
        
    except Exception as ex: 
        if str(ex) != 'Message to delete not found':
            await exceptions("test.py", 'test_menu', ex)

#============================================================================================## ВЫЗОВ МЕНЮ ТЕСТОВ
@dp.callback_query_handler(CHAT_PRIVATE, text="test_menu")
async def inline_menu_test(call: CallbackQuery):
    try: await test_menu(call.message.chat.id, call.message.message_id), await call.answer()
    except Exception as ex: await exceptions("test.py", 'inline_menu_test', ex)